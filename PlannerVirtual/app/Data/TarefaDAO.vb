...
Public Class TarefaDAO
        ...
        Catch ex As CategoriaNaoEncontradaException
            Using cn = New SQLiteConnection(sConnectionString)
                ...
                Using objCommand As SQLiteCommand = cn.CreateCommand()
                    objCommand.CommandText = "INSERT INTO Categorias (nome , cor) VALUES ('" & tarefa.nome & "', " & Color.Red.ToArgb & ")"
    ...
    Public Sub deletar(nome As String) Implements ITarefaDAO.deletar
        Using cn = New SQLiteConnection(sConnectionString)
            cn.Open()
            Using objCommand As SQLiteCommand = cn.CreateCommand()
    ...
    Public Function listar() As List(Of Tarefa) Implements ITarefaDAO.listar
        ...
        Dim listaTarefas As List(Of Meta) = New List(Of Meta)
        ...
        Using cn = New SQLiteConnection(sConnectionString)
            ...
            Using cmd = New SQLiteCommand(sql, cn)
                Using dr = cmd.ExecuteReader()
                    If dr.HasRows Then
                        While dr.Read()
                            Dim tarefa As Tarefa = New Tarefa(dr("nome"), Color.FromArgb((dr("cor"))))
    ...
    Public Function consultar(nome As String) As Tarefa Implements ITarefaDAO.consultar
        Using cn = New SQLiteConnection(sConnectionString)
            cn.Open()
